  ...
  initCommands(){
    ...
    cmdPlugin.registerCommand('delete', {
      enable: function(graph) {
        ...
        return mode === 'edit' && selectedItems && selectedItems.length > 0 && items && items.length > 0 && items[0] !== 'start' && items[0] !== 'exit';
      },
    ...
    cmdPlugin.registerCommand('paste', {
      enable: function(graph) {
        ...
        const items = graph.get('selectedItems');
        console.log(items);
        return mode === 'edit' && cmdPlugin.get('_command').clipboard.length > 0 && items && items.length > 0 && items[0] !== 'start' && items[0] !== 'exit';
      },
      method: function(graph) {
        const manager = cmdPlugin.get('_command');
